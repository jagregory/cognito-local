  ...
  public async sendEmail(
    ...
    await this.transporter.sendMail({
      ...
      html: `<div style="font-family:system-ui;padding:20px;">
        ...
        <p>${body}</p>
        ${message.__code ? `<p style="font-size:24px;font-weight:bold;letter-spacing:4px;margin:20px 0;">${message.__code}</p>` : ""}
        <p style="color:#999;font-size:12px;">User: ${user.Username}</p>
      </div>`,
    });
  }
